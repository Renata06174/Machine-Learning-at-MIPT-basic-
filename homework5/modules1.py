...
Original file is located at
    https://colab.research.google.com/github/Renata06174/ml-mipt/blob/master/homeworks_basic/assignment0_05_NN_from_scratch/modules.ipynb
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(gradOutput.shape[0]):
          ...
          local_derivative[i] -= local_softmax[i].reshape(-1,1).dot(local_softmax[i].reshape(-1,1).T)
        self.gradInput = np.matmul(gradOutput[:,None,:],(local derivative))[:,0,:]
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        local_derivative = np.zeros((gradOutput.shape[0], gradOutput.shape[1], gradOutput.shape[1]))
        local_logsoftmax = self.output
        normal_input = np.subtract(input, input.max(axis=1, keepdims=True))
        E = n.exp(normal_input) / np.sum(np.exp(normal_input), axis = 1, keepdims = True)
        for i in range(gradOutput.shape[0]):
          local_derivative[i] = np.diag(E[i].shape)
          local_derivative[i] -= E[i]
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
          ...
          self.output = (input - batch_mean)/sqrt(batch_variance + self.EPS)
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        dmean = np.sum(np.multiply(dxhat, -1. / np.sqrt(batch_variance + self.EPS)), axis=0)
        self.gradInput = np.multiply(dxhat, 1. / np.sqrt(batch_variance + self.EPS)) + np.multiply(dvar, 2 * (input - batch_mean) / m) + dmean / m
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        self.mask = np.random.binomial(1, p, size=input.shape)
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = 1 / (1 - self.p) * (1 - self.mask) * gradOutput
        return self.gradInput
...
def adam_optimizer(variables, gradients, config, state):  
    ...
    for current_layer_vars, current_layer_grads in zip(variables, gradients): 
        for current_var, current_grad in zip(current_layer_vars, current_layer_grads):
            ...
            #current_var -= ...
            np.add((1 - beta1) * current_grad, 
                   beta1 * var_first_moment, 
                   out=var_first_moment)
            np.add((1 - beta2) * current_grad**2, 
                   beta2 * var_second_moment, 
                   out=var_second_moment)
            current_var -= lr_t * var_first_moment / (np.sqrt(var_second_moment) + eps)
